    ...
    <section style={{ padding: "140px 24px", background: "#000", position: "relative" }}>
      ...
      <div style={{ maxWidth: 1040, margin: "0 auto" }}>
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} style={{ textAlign: "center", marginBottom: 20 }}>
          <span style={{ fontSize: 11, letterSpacing: "0.14em", textTransform: "uppercase", color: "#3a3a3a", fontFamily: B }}>Témoignages</span>
        </motion.div>
        ...
        <motion.h2 initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}
          ...
          Ce que les agences
          <br /><span style={{ color: "#2a2a2a" }}>partenaires disent.</span>
        </motion.h2>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginBottom: 48 }}>
          {testimonials.map((t, i) => (
            <motion.div key={i}
              ...
              style={{ background: "#080808", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 18, padding: "26px 24px", display: "flex", flexDirection: "column" }}>
              <div style={{ display: "flex", gap: 1.5, marginBottom: 18 }}>
                {[...Array(5)].map((_, j) => (
                  <motion.div key={j}
              ...
              <p style={{ fontSize: 13, color: "#5a5a5a", lineHeight: 1.8, flex: 1, marginBottom: 20, fontFamily: B, fontStyle: "italic" }}>"{t.quote}"</p>
            ...
            <motion.div key={f.name}
              ...
              <div style={{ display: "flex", gap: 14, alignItems: "flex-start", marginBottom: 20 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: "#2a2a2a", marginTop: 2, fontFamily: B }}>{f.bg}</div>
                </div>
              </div>
              <blockquote style={{ fontSize: 13, color: "#4a4a4a", lineHeight: 1.75, fontStyle: "italic", borderLeft: "2px solid rgba(255,255,255,0.06)", paddingLeft: 14, fontFamily: B }}>
                "{f.quote}"
              </blockquote>
            </motion.div>
          ))}
