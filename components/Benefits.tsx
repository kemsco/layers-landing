  ...
  return (
    <motion.div ref={ref} onMouseMove={onMove} onMouseLeave={onLeave}
      ...
      <div style={{ position: "relative", ...style }}>
        <motion.div style={{
          position: "absolute", inset: 0, borderRadius: "inherit", pointerEvents: "none", zIndex: 1, opacity: 0.06,
...
const blocks = [
  ...
  },
  {
    ...
    desc: "Seats illimités, projets illimités, crédits alloués librement par client. Le suivi GEO de 40 comptes tourne en parallèle — sans que ça vous coûte 40× plus.",
    stat: "∞", statSub: "clients en simultané", large: false,
  },
  {
    n: "04", icon: "◇",
    tag: "Crédibilité",
    title: "Vous parlez de GEO avec data. Pas avec des généralités.",
    desc: "Chaque recommandation est sourcée sur des études réelles et validée par des tests. Vos clients reçoivent des insights défendables.",
    ...
    <section style={{ padding: "140px 24px", background: "#000", position: "relative" }}>
      ...
      <div style={{ maxWidth: 1040, margin: "0 auto", position: "relative", zIndex: 1 }}>
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} style={{ textAlign: "center", marginBottom: 20 }}>
          <span style={{ fontSize: 11, letterSpacing: "0.14em", textTransform: "uppercase", color: "#3a3a3a", fontFamily: B }}>Bénéfices</span>
        </motion.div>
        ...
        <motion.h2 initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}
          ...
          <br /><span style={{ color: "#2a2a2a" }}>pour votre agence.</span>
        </motion.h2>
        ...
        {/* Bento : large card left + 3 small right */}
        <div style={{ display: "grid", gridTemplateColumns: "1.35fr 1fr", gridTemplateRows: "auto auto", gap: 14 }}>
          ...
          <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}
            ...
            <TiltCard style={{ height: "100%", background: "#080808", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 20, padding: "36px 34px", display: "flex", flexDirection: "column" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
                <span style={{ fontSize: 28, color: "#333" }}>{main.icon}</span>
                <span style={{ fontSize: 10, color: "#444", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)", padding: "4px 12px", borderRadius: 99, fontFamily: B, letterSpacing: "0.08em", textTransform: "uppercase" }}>{main.tag}</span>
              </div>
              <h3 style={{ fontFamily: D, fontSize: 22, fontWeight: 700, letterSpacing: "-0.03em", marginBottom: 16, lineHeight: 1.2 }}>{main.title}</h3>
              <p style={{ fontSize: 14, color: "#4a4a4a", lineHeight: 1.8, marginBottom: 32, fontFamily: B }}>{main.desc}</p>

              {/* Visual mockup: report preview */}
              <div style={{ background: "#0d0d0d", borderRadius: 12, padding: "16px 18px", marginBottom: 24, border: "1px solid rgba(255,255,255,0.05)", flex: 1 }}>
                <div style={{ fontSize: 9, color: "#2a2a2a", marginBottom: 14, fontFamily: B, textTransform: "uppercase", letterSpacing: "0.1em" }}>Aperçu rapport — Acme Corp</div>
                {[
                  ...
                  <div key={item.label} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 9 }}>
                    <span style={{ fontSize: 10, color: "#444", width: 110, flexShrink: 0, fontFamily: B }}>{item.label}</span>
                    <div style={{ flex: 1, height: 3, background: "#1a1a1a", borderRadius: 2 }}>
                      <motion.div
                        ...
                        style={{ height: 3, borderRadius: 2, background: item.color }}
                      />
                    </div>
                    <span style={{ fontSize: 10, color: item.color, fontFamily: D, fontWeight: 700, width: 28, textAlign: "right" }}>{item.val}%</span>
                  </div>
                ))}
                <div style={{ marginTop: 16, padding: "10px 12px", background: "rgba(110,231,183,0.04)", border: "1px solid rgba(110,231,183,0.1)", borderRadius: 8 }}>
                  ...
                  <p style={{ fontSize: 11, color: "#4a4a4a", fontFamily: B, lineHeight: 1.6 }}>Renforcer les mentions marque sur les contenus long-form pour améliorer la citation Gemini de +12 pts.</p>
                </div>
              </div>
              ...
              <div style={{ borderTop: "1px solid rgba(255,255,255,0.05)", paddingTop: 22 }}>
                ...
                <div style={{ fontSize: 12, color: "#444", marginTop: 3, fontFamily: B }}>{main.statSub}</div>
              </div>
            </TiltCard>
          </motion.div>
          ...
          {rest.map((b, i) => (
            <motion.div key={b.n} initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.1 + i * 0.1 }}>
              <TiltCard style={{ background: "#080808", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 20, padding: "26px 26px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                  <span style={{ fontSize: 18, color: "#333" }}>{b.icon}</span>
                  <span style={{ fontSize: 10, color: "#444", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)", padding: "3px 10px", borderRadius: 99, fontFamily: B, textTransform: "uppercase", letterSpacing: "0.08em" }}>{b.tag}</span>
                </div>
                <h3 style={{ fontFamily: D, fontSize: 16, fontWeight: 700, letterSpacing: "-0.03em", marginBottom: 10, lineHeight: 1.3 }}>{b.title}</h3>
                <p style={{ fontSize: 13, color: "#4a4a4a", lineHeight: 1.75, marginBottom: 20, fontFamily: B }}>{b.desc}</p>
                <div style={{ borderTop: "1px solid rgba(255,255,255,0.05)", paddingTop: 14 }}>
                  <div style={{ fontFamily: D, fontSize: 28, fontWeight: 700, letterSpacing: "-0.05em" }}>{b.stat}</div>
                  <div style={{ fontSize: 11, color: "#444", marginTop: 2, fontFamily: B }}>{b.statSub}</div>
                </div>
              </TiltCard>
            </motion.div>
          ))}
