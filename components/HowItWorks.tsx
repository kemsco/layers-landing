    ...
    <section style={{ padding: "140px 48px", background: "#020202", position: "relative" }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} style={{ textAlign: "center", marginBottom: 20 }}>
          <span style={{ fontSize: 11, letterSpacing: "0.14em", textTransform: "uppercase", color: "#444", fontFamily: B }}>Process</span>
        </motion.div>
        ...
        <motion.h2 initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}
          ...
          <br /><span style={{ color: "#555" }}>d'une heure.</span>
        </motion.h2>

        <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.15 }}
          style={{ textAlign: "center", fontSize: 15, color: "#666", marginBottom: 80, fontFamily: B, lineHeight: 1.75 }}>
          Trois étapes. Pas de développement. Pas de formation.
        </motion.p>

        <div style={{ position: "relative" }}>
          {/* Vertical connector */}
          <div style={{ position: "absolute", left: 27, top: 56, bottom: 56, width: 1, background: "linear-gradient(180deg, rgba(255,255,255,0.08), rgba(255,255,255,0.02))", zIndex: 0 }} />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {steps.map((step, i) => (
              ...
              >
                {/* Step number */}
                <div style={{ flexShrink: 0, width: 54, height: 54, background: "#080808", border: "1px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1, position: "relative" }}>
                ...
                <motion.div
                  ...
                  style={{ flex: 1, background: "#080808", border: "1px solid rgba(255,255,255,0.06)", padding: "26px 28px", marginBottom: 0, transition: "border-color 0.3s" }}
                >
                  <h3 style={{ fontFamily: D, fontSize: 18, fontWeight: 700, letterSpacing: "-0.03em", marginBottom: 12, color: "#fff" }}>{step.title}</h3>
                  <p style={{ fontSize: 14, color: "#666", lineHeight: 1.75, fontFamily: B, marginBottom: 18 }}>{step.desc}</p>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {step.detail.map(tag => (
                      <span key={tag} style={{ fontSize: 11, color: "#444", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", padding: "3px 10px", fontFamily: B }}>
                        {tag}
                      </span>
                    ))}
                  </div>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.5 }}
          ...
          <div style={{ display: "inline-flex", alignItems: "center", gap: 10, padding: "14px 24px", background: "#080808", border: "1px solid rgba(255,255,255,0.07)" }}>
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
              <circle cx="7" cy="7" r="6" stroke="rgba(255,255,255,0.2)" />
              <path d="M7 4v3.5l2 2" stroke="rgba(255,255,255,0.5)" strokeWidth="1.2" strokeLinecap="round" />
            </svg>
